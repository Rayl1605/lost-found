...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES['item_image']) && $_FILES['item_image']['error'] == 0) {
        ...
        if (!in_array(strtolower($ext), $allowed)) {
            ...
            if ($filesize > 5 * 1024 * 1024) { 
                ...
                $new_filename = time() . "_lost_" . $filename;
                $upload_dir = "uploads/";
    ...
    if ($msg_type != "error") {
        $sql = "INSERT INTO lost_items (item_name, category, location, date_lost, description, reporter_name, image_path, user_id) 
                VALUES ('$item_name', '$category', '$location', '$date_lost', '$description', '$reporter_name', '$image_path', '$user_id')";

        if ($conn->query($sql) === TRUE) {
            $message = "Report submitted successfully! We will notify you if we find a match.";
            $msg_type = "success";
        } else {
...
<body class="bg-gray-100 min-h-screen py-10">
    ...
    <div class="container mx-auto px-4 max-w-2xl">
        ...
        <div class="flex items-center gap-4 mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Report Lost Item</h1>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div class="bg-blue-600 p-6 text-white flex items-center justify-between">
                <div>
                    ...
                    <p class="text-blue-200 text-sm">Please provide as much detail as possible.</p>
                </div>
                <i class="fas fa-search text-3xl opacity-50"></i>
            </div>

            <form action="report_lost.php" method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Item Name</label>
                        <input type="text" name="item_name" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. Black Leather Wallet">
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Category</label>
                        <select name="category" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Date Lost</label>
                    <input type="date" name="date_lost" required class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500">
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Location Lost (Approx)</label>
                    <div class="relative">
                        <span class="absolute left-4 top-3.5 text-gray-400"><i class="fas fa-map-marker-alt"></i></span>
                        <input type="text" name="location" required class="w-full bg-gray-50 border border-gray-200 rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="e.g. 2nd Floor Library">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Description</label>
                    <textarea name="description" rows="4" class="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Describe the item (color, brand, distinguishing marks)..."></textarea>
                </div>
                ...
                <div class="pt-4 flex gap-4">
                    ...
                    <button type="submit" class="flex-1 py-3 rounded-xl bg-blue-600 text-white font-bold shadow-lg hover:bg-blue-700 hover:shadow-xl transition transform active:scale-95">Submit Report</button>
                </div>
